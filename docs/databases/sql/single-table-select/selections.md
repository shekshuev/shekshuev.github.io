...
<Conversation :phrases="[
    {
        name: 'Петр',
        position: 'left',
        text: 'Федор, хочу видеть по одному представителю от каждого сословия. Но не просто название, а имя и фамилию! Хочу лично поговорить с одним дворянином, одним крестьянином и одним иноземцем.',
        photo: petr
    },
    {
        name: 'Федор',
        position: 'right',
        text: 'Если я использую обычный DISTINCT, я потеряю имена... А если оставлю имена, появятся дубликаты сословий.',
        photo: ivan
    },
    {
        name: 'Алексей',
        position: 'right',
        text: 'В нашей базе (PostgreSQL) есть хитрость — DISTINCT ON. Она оставляет одну строку для группы, а остальные данные подтягивает.',
        photo: alexey
    }
]"/>

Допустим, нам нужно выбрать **по одному** рекруту каждого сословия. Причем мы хотим, чтобы это был **самый высокий** представитель.
